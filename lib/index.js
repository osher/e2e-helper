...
// default parameters for istanbul e2e coverage
const coverDefaults = 
  { args: ['./node_modules/istanbul/lib/cli', 'cover', '--handle-sigint', '--dir', './coverage/e2e-test', '-x', '**/test-e2e/**']
  }
...
function initCtx(options) {
    ...
    const myDefaults = assign({}, defaults)
    
    if (process.env.COVER) {
        // istanbul need to be called with a .js suffix
        if (!options.svc.match(/\.js$/)) {
            options.svc += '.js'
        }
        assign(myDefaults, coverDefaults)
    }

    options = assign(myDefaults, options)
    ...
    msg = (
       ...
       || !fs.existsSync(options.svc + '.js')          
          && !fs.existsSync(path.join( options.cwd, options.svc)) 
    ...
    // Since the extend method uses the '=' operator, we're getting a reference to the
    // default.args array. this is NOT what we want since we want to be able to
    // change the options.args array without changing the default one.
    // TBD consider replace the extend method with a deep-extend one and delete following lines
    options.args = options.args.slice(0)
    if (process.env.COVER) {
        if (process.env.COVER_IGNORE) {
            process.env.COVER_IGNORE
              .split(',')
              .map((pattern) => pattern.trim())
              .forEach((f) => options.args.push('-x', f))
        }
        options.args.push(options.svc)
    } else {
